...
const PROGRESS_FILE  = path.join(ROOT, `progress-${TENANT}.json`);
const PLAYERS_DIR    = path.join(ROOT, 'players');
const SKIPPED_FILE   = path.join(ROOT, 'seasons-skipped.json');
const INVALID_FILE   = path.join(ROOT, 'seasons-invalid.json');
const SHARDS_DIR     = path.join(ROOT, 'players-index');
...
const QUEUE_PRIORITY_FILE = path.join(ROOT, `queue-${TENANT}-priority.json`);
const QUEUE_BACKLOG_FILE  = path.join(ROOT, `queue-${TENANT}-backlog.json`);
  ...
  while (true) {
    ...
    if (!res.ok) {
      ...
      if (res.status >= 500 && attempts <= 3) {
        console.warn(`  ⚠ HTTP ${res.status} for ${operationName} — retrying in 10s (attempt ${attempts}/3)`);
        await delay(10000);
        continue;
      }
