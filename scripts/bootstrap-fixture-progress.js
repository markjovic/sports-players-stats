...
const ZERO_FILE     = path.join(ROOT, 'zero-team-seasons.json');
...
const noVenueFile = path.join(ROOT, 'no-venue-seasons.json');
const noVenue = [];
for (const [seasonId, season] of Object.entries(seasons)) {
  const gameFile = path.join(GAMES_DIR, `${seasonId}.json`);
...
const noLadderFile = path.join(ROOT, 'no-ladder-seasons.json');
const noLadder = [];
for (const [seasonId, season] of Object.entries(seasons)) {
