// scripts/discover-game-backfill.js
//
// SECOND capture path for game rosters, added 2026-08-11 after a discovery that
// invalidated an assumption the whole roster pipeline rested on.
//
// spectator.playhq.com is the LIVE-SCORING service: it only holds games scored in
// real time at the venue. Games scored on paper and entered afterwards exist in
// PlayHQ's CANONICAL record with a full box score and are simply absent from
// spectator, which answers "game could not be found or was not electronically
// scored" for them. Our entire roster capture read spectator, so paper-scored
// games have been invisible since day one — this is NOT an era or a rollout, it
// is a scoring-METHOD split. Proven 2026-08-11: of five sample games from seasons
// at 0% spectator coverage, four returned full lineups here (the fifth 404s —
// deleted upstream).
//
// This walks games spectator RETIRED as misses (spcm set, no spc) and asks
// api.playhq.com/graphql instead, with the `gameView` operation copied VERBATIM
// from a live browser request. It writes the same p[] roster shape and marks the
// game `dg: 1` — a DISTINCT flag, never spc, so provenance stays honest and spcm
// survives as the record that spectator genuinely could not serve it.
//
// Two advantages over the spectator path: profiles arrive as FULL uuids with
// first and last names, so new players are stubbed already named instead of
// waiting for the matrix; and fill-in / anonymous participants carry no profile
// id, so they are counted and skipped rather than guessed at.
//
// Session handling, doFetch, the commit-window player phase, stubbing and
// gitCommit are unchanged from spectator-backfill.js, which inherits them
// verbatim from nightly-crawl.js.
//
// Usage:
//   node scripts/discover-game-backfill.js --dry-run
//   node scripts/discover-game-backfill.js --max-games=50000
//   node scripts/discover-game-backfill.js --season=<sid> --concurrency=25
...
// --heal-dangling is a spectator-path mode and is NOT supported here (its queue
// branch below is inherited dead code; this constant keeps it unreachable).
const HEAL_DANGLING = false;
...
// Concurrency: this is the MAIN api, not spectator. Kept deliberately modest —
// the nightly crawl depends on this same endpoint and a 429/403 regime here would
// damage far more than this backfill.
const CONCURRENCY_GAMEVIEW = Number((process.argv.find(a => a.startsWith('--concurrency=')) || '').replace('--concurrency=', '')) || 25;
...
// discoverGame players -> the SAME shape parseSpectatorPlayers returns, so every
// downstream delta / stub / write path is untouched. Fill-in and anonymous
// participants have no profile and are dropped (counted by the caller).
function parseGameViewPlayers(players) {
  const out = [];
  for (const p of (players || [])) {
    const prof = p && p.player && p.player.profile;
    if (!prof || !prof.id) continue;
    const stats = {};
    for (const s of (p.statistics || [])) {
      const v = s && s.type && s.type.value;
      if (v) stats[v] = s.count || 0;
    }
    out.push({
      profileID: prof.id,
      name: [prof.firstName, prof.lastName].filter(Boolean).join(' ').trim(),
      playerNumber: p.playerNumber || null,
      pts:   stats['TOTAL_SCORE']   || 0,
      pt3:   stats['3_POINT_SCORE'] || 0,
      fouls: stats['TOTAL_FOULS']   || 0,
    });
  }
  return out;
}
...
async function gqlSpectator(gameId) {
  ...
  try {
    ...
    if (body.errors) {
      ...
      // The live message, captured 2026-08-11:
      //   "game could not be found or was not electronically scored"
      // — PlayHQ's definitive answer for this endpoint, covering both causes in one
      // string. It is PERMANENT either way: the spectator API will never serve that
      // game. Note "could not be found" does NOT match a naive /not found/ pattern
      // (the words are separated), which is exactly why the first version of this
      // regex misclassified 200/200 of them as transient.
...
function buildBackfillQueue(sportIndex) {
  ...
  const tallies = {
    ...
    queuedFinalSt: 0, queuedScoreNoSt: 0, queuedEmptyList: 0, partialListLeftAlone: 0, queuedPartialList: 0, tooRecent: 0, dgSet: 0, profileOnly: 0, notTried: 0, notFinal: 0, retiredMisses: 0, queuedDangling: 0, danglingIds: 0,
  ...
  for (const fname of files) {
    ...
    for (const [gameId, g] of Object.entries(gf.games || {})) {
      if (!g) continue;
      tallies.games++;
      // ── Selection: games spectator RETIRED, i.e. it could not serve them ──
      // Requires spcm (spectator was actually asked and failed) so this never
      // races ahead of the spectator sweep on games it has not tried yet.
      if (g.dg)                                 { tallies.dgSet++;         continue; }
      ...
      // profileOnly entries have no real game behind them anywhere — discoverGame
      // cannot serve them either, and synthesize-missing-games owns that population.
      if (g.profileOnly)                        { tallies.profileOnly++;   continue; }
      if (MISS_ATTEMPT_LIMIT > 0 && (g.dgm || 0) >= MISS_ATTEMPT_LIMIT) { tallies.retiredMisses++; continue; }
      if (!(g.spcm > 0))                        { tallies.notTried++;      continue; }
      if (g.st !== 'FINAL')                     { tallies.notFinal++;      continue; }
...
async function main() {
  ...
  let spectatorHits = 0, spectatorMiss = 0, p3Done = 0, transientFail = 0, skippedNoProfile = 0;
  ...
  const p3Tasks = needsSpectator.map(({ gameId, seasonId, rn, gradeId, gradeName, homeTid, awayTid, homeScore, awayScore }) => async () => {
    const res = await gqlGameView(gameId);
    p3Done++;
    ...
    if (!game?.statistics) {
      ...
      if (!DRY_RUN) {
        ...
        if (gfm.games[gameId]) {
          // dgm, not spcm: this path's failures must never overwrite the spectator
          // service's own record of what it could and could not serve.
          gfm.games[gameId].dgm = (gfm.games[gameId].dgm || 0) + 1;
    ...
    const homePlayers = parseGameViewPlayers(game.statistics?.home?.players);
    const awayPlayers = parseGameViewPlayers(game.statistics?.away?.players);
    skippedNoProfile += ((game.statistics?.home?.players || []).length - homePlayers.length)
                      + ((game.statistics?.away?.players || []).length - awayPlayers.length);
    ...
    if (gf.games[gameId] && !DRY_RUN) {
      ...
      // dg:1, never spc — provenance stays honest, and spcm is PRESERVED as the
      // standing record that the live-scoring service could not serve this game.
      gf.games[gameId].dg = 1;
      markGameDirty(seasonId);
    }
  });
  ...
  const wrappedTasks = p3Tasks.map(t => async () => {
    ...
    if (sinceCommit >= COMMIT_EVERY_GAMES) {
      ...
      await tryPeriodicCommit(`discover-game-backfill: progress ${p3Done}/${needsSpectator.length} (${spectatorHits} hits, ${totalStubbed} stubbed)`, ['games/', 'players/']);
      evictCleanSeasons();
    }
  });

  await runPool(wrappedTasks, CONCURRENCY_GAMEVIEW);
  ...
  if (flushedSpc > 0) {
    await gitCommit(`discover-game-backfill: dg+p[] written for ${spectatorHits} games`, ['games/']);
  }
  console.log();
  ...
  await gitCommit(
    `discover-game-backfill: ${spectatorHits} games filled, ${totalStubbed} new players stubbed (this run)`,
    ['players/']
  );
